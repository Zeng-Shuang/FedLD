...
def bn_average_weights_weighted(w, avg_weight):
    """
    Returns the average of the weights except BN layers.
    """
    w_avg = copy.deepcopy(w[0])
    weight = torch.tensor(avg_weight)
    agg_w = weight/(weight.sum(dim=0))
...
def get_head_agg_weight(num_users, Vars, Hs, *args, **kwargs):
    ...
    for i in range(num_users):
        ...
        evals, evecs = torch.linalg.eig(torch.tensor(p_matrix))#, eigenvectors=True
        ...
        for ii in range(num_users):
            #if evals[ii,0] >= 0.01:
            evals_clone = evals[ii].clone()
            evals_ii = torch.real(evals_clone)
            #print('eeeeeeeeeeeeeeeeeeeeeee',evals_ii)
            if evals_ii >= 0.01:
                p_matrix_new += evals[ii]*torch.mm(evecs[:,ii].reshape(num_users,1), evecs[:,ii].reshape(1, num_users))
...
def get_parameter_values(model):
    #params_on_device = [param.to(args.device) for param in model.parameters()]
    #parameter = torch.cat([torch.reshape(param.data, (-1,)) for param in params_on_device]).clone().detach()
